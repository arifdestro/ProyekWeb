...
if(isset($_POST['tambah_daftar'])){
    ...
    if($ID_JENIS_LOMBA=='J0001'){
        ...
        if($size < 3132210 && $size != 0){ 
            $id = rand(0,100);
            $uniq = uniqid($id,true);
            move_uploaded_file($file_temporary, 'assets/rekom/'.$uniq.'.'.$ekstensi); //untuk upload file
            $id2 = rand(0,100);
            ...
            if($query) {
                echo '<script>alert("Berhasil menambahkan data."); document.location="pendaftaran_saya.php";</script>';
                
            }else{
                echo '<script>alert("Gagal menambahkan data."); document.location="tambah_daftar.php";</script>';
            }
        }else{
            echo '<script>alert("Foto terlalu besar."); document.location="tambah_daftar.php";</script>';

        }
        }else{
            echo '<script>alert("Ekstensi tidak diperbolehkan."); document.location="tambah_daftar.php";</script>';
        }
